  ...
  return (
    <section id="projects" className="py-24 px-6 relative">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute left-1/2 -translate-x-1/2 top-20 w-[600px] h-[400px] bg-indigo-600/5 rounded-full blur-3xl" />
      </div>
        ...
        {projects
          ...
          .map((p) => (
            <motion.div
              ...
              className="mb-8 relative rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-indigo-500/10 via-violet-600/5 to-transparent backdrop-blur-sm p-8 overflow-hidden group hover:border-indigo-500/50 transition-all"
            >
                ...
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-indigo-400 text-sm font-medium">{p.subtitle}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {p.tags.map((t) => (
                      <span
                        key={t}
                        className="px-3 py-1 text-xs font-semibold rounded-lg bg-indigo-500/15 border border-indigo-500/25 text-indigo-300"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold bg-gradient-to-r from-indigo-500 to-violet-600 text-white shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:scale-105 transition-all"
                  >
                ...
                <div className="space-y-3">
                  {p.highlights!.map((h, j) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 rounded-xl bg-white/3 border border-white/5 px-4 py-3"
                    >
          ...
          {projects
            ...
            .map((p, i) => (
              <motion.div
                ...
                className="rounded-2xl border border-white/8 bg-white/3 backdrop-blur-sm p-6 flex flex-col hover:border-indigo-500/30 hover:bg-white/5 transition-all group"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  {/* Case Study badge */}
                  <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400">
                    <BookOpen size={10} />
                    Case Study
                  </span>
                </div>
                ...
                <p className="text-xs text-indigo-400 font-medium mb-3">{p.subtitle}</p>
